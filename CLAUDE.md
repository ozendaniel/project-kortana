...
- CRITICAL: Seamless REST API is unreliable for menus (2026-04-03). The `enhanced_feed` + `restaurant_gateway/feed` endpoints return items from multiple inactive menu sets (ghost items like "Beef Tendon") AND miss items that ARE on the website (like "Mango Jumbo Shrimp"). Neither `hideUnavailableMenuItems` nor category deduplication fixes this. The website uses a protobuf BFF endpoint (`menubff`) + client-side filtering. DOM scraping requires SPA auth (not just API auth) and delivery address in range. Bulk Seamless populate script stopped at 1,647 restaurants — all data unreliable. DoorDash API IS reliable (verified: 0 ghost items, matches website exactly).
...
**What's next:**
1. **BLOCKER: Fix Seamless menu data** — REST API is unreliable (returns ghost items + misses real items). Need to solve Seamless SPA DOM scraping. The SPA requires full browser auth (not just API token) and delivery address in range. Approaches to try: (a) use Seamless portal login via Settings page to establish full SPA session, then scrape, (b) intercept/decode the menubff protobuf endpoint, (c) use DoorDash as source of truth and match Seamless items against it using the API data + LLM matching (accepting some inaccuracy). See detailed investigation notes in memory.
...
3. Re-run Seamless menu population with corrected approach (all 1,647 previously populated restaurants have unreliable data)
4. Run cross-platform item matching after both menus are populated
...
| Platform API approach | DoorDash: GraphQL (reliable, no ghost items). Seamless: REST API is UNRELIABLE for menus — returns ghost items + misses real items. Need DOM scraping or protobuf decoding for accurate Seamless menus. | DoorDash storepageFeed returns exactly what's on the website. Seamless enhanced_feed returns multiple inactive menu sets with no API signal to filter. Website uses menubff protobuf + client-side filtering. |
| Menu data source of truth | DoorDash | DoorDash API verified accurate (0 ghost items, prices match website). Seamless API unreliable. Match Seamless items against DoorDash menu items for comparison. |
