...
- **Seamless concurrent DOM scraping (2026-04-05):** populate-seamless-menus.ts now supports `--concurrency N` flag for parallel restaurant scraping. Each worker opens its own fresh tab. Health checks and keep-alive run between batches. `--concurrency 2` recommended on 16GB Surface laptop (~1 restaurant/min per worker). Currently in progress.
...
**What's next:**
1. **Seamless DOM scraping in progress** — running with `--concurrency 2 --resume --sustained --skip-match`. ~4,467 restaurants to process. Monitor progress via DB: `SELECT COUNT(DISTINCT mi.restaurant_id) FROM menu_items mi JOIN restaurants r ON r.id = mi.restaurant_id WHERE mi.platform = 'seamless' AND mi.created_at > NOW() - INTERVAL '24 hours'`. Note: background task output files go stale — always check DB directly for progress.
2. **Fix Dim Sum Palace DOM scraping** — "View More Items" button not handled, causing incomplete menus on complex restaurants. Must fix before trusting all SL menu data. Seamless populate is running but data for restaurants with this button will be incomplete.
...
7. **Monitor Railway Postgres storage** — check: `SELECT pg_size_pretty(pg_database_size(current_database()))`
