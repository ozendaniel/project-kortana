...
| Seamless DOM scraping | Headful Chrome + fresh tab + MutationObserver + per-item h3 tracking | HeadlessChrome gets empty menus (API detects UA). Headful Chrome + MutationObserver captures virtualized items during scroll. Per-item category via h3 position. `item_search` endpoint supplements with ~5% extra items (needs portal login). |
