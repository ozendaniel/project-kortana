...
- **Seamless DOM scraping SOLVED (2026-04-04):** Live-tested on Dim Sum Palace (SL ID 1387494, 33 W 33rd St). DOM scraping extracts exactly what the user sees — no ghost items. Key findings: (1) Must use a FRESH TAB per restaurant (stale SPA state on reused tabs breaks rendering). (2) Seamless virtualizes the menu — category headers (`menuVirtualizedSection`) and items (`.menuItem`) are in SEPARATE DOM branches, not parent-child. (3) Track "current category" by viewport position of h3 headers during scroll. (4) Items live in `[data-testid="regular-sections"]`. (5) Item IDs extracted from `data-testid="Item{id}-{category}"`. (6) Delivery address must be in range or SPA redirects to a different restaurant. (7) Use `page.keyboard` for address input (React re-renders detach element references). (8) `networkidle` wait needed for initial SPA load. API auth tokens in localStorage work — SPA's "Sign in" nav text is cosmetic, not functional.
...
**What's next:**
1. **Re-run Seamless menu population via DOM scraping** — `getMenuFromDOM()` now works (proven on Dim Sum Palace: 236 items, 17 categories, real item IDs). Need to update `populate-seamless-menus.ts` to use fresh-tab DOM scraping with per-restaurant address management. All 1,647 previously populated menus have unreliable API data — must be re-scraped. Address must be in each restaurant's delivery range; out-of-range restaurants need address lookup from DB coordinates.
2. **Run DoorDash menu bulk population** — script ready (populate-doordash-menus.ts), tested on 3 restaurants. DoorDash API data IS reliable (verified: 0 ghost items). Run `--matched-only` first (251 restaurants, ~50 min), then full run (~2,028 restaurants, ~10 hours)
3. Run cross-platform item matching after both menus are populated
4. Test Dim Sum Palace cross-platform comparison — DD data vs DOM-scraped SL data for seamless.com/menu/dim-sum-palace-33-w-33rd-st-new-york/1387494
5. Implement savings tracking (log comparisons/orders to DB)
6. Enrich DoorDash restaurants with real addresses via `--enrich` flag
7. **Monitor Railway Postgres storage** — currently 146 MB / 500 MB. Check: `SELECT pg_size_pretty(pg_database_size(current_database()))`
...
| Seamless DOM scraping | Fresh tab + viewport-based category tracking + incremental scroll | Seamless virtualizes menu (items enter/leave DOM). Category headers and items are in SEPARATE DOM branches. Must track "current category" by h3 viewport position during scroll. Items in `[data-testid="regular-sections"]`, IDs from `data-testid="Item{id}"`. Delivery address must be in range. |
...
- **Menu scraping (DOM):** REST API is unreliable for menus (ghost items). `getMenuFromDOM()` opens a FRESH TAB, navigates to `/menu/{id}`, scrolls incrementally to collect items from the virtualized DOM. Category headers in `.menuVirtualizedSection`, items in `[data-testid="regular-sections"]` — they're SEPARATE DOM branches (not parent-child). Track current category by viewport position of h3. Item IDs from `data-testid="Item{id}-{category}"`. Delivery address must be in range. Proven: 236 items, 17 categories from Dim Sum Palace.
