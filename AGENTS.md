...
Dan Ozen — building solo. PE background (O3 Industries). NYC-based.
...
- **DoorDash GraphQL URL routing discovered**: All DD operations require `/graphql/{operationName}?operation={operationName}` (not plain `/graphql`). Also requires `x-csrftoken`, `apollographql-client-name`, `x-channel-id` headers for restricted operations like `itemPage`. See `project_doordash_graphql_quirks.md` in agent memory.
...
- **Live-only fees + typed errors + preflight**: Comparison engine no longer falls back to cached `platform_fees` or hardcoded DB estimates. Every `/api/compare` hits both adapters' live `getFees()`. Adapters throw `LiveFeeError` with a `reason` enum (`session_expired`, `adapter_unavailable`, `out_of_delivery_range`, `item_unavailable`, `unknown`). Comparison result per platform is either `{kind: 'ok', ...PlatformComparison}` or `{kind: 'error', reason, message, canRetry, orderUrl}`. DD `getFees` now calls `setDeliveryAddress()` internally (already captured mutation `updateConsumerDefaultAddressV2`) to snap to nearest saved address before building the cart. New routes: `GET /api/compare/preflight?restaurantId=...` returns per-platform readiness + DD account addresses; `POST /api/compare/refresh` clears the in-memory fee cache. Frontend `ComparisonView` now shows an address confirmation block, surfaces per-platform error cards with "Reconnect" deep-links to `/settings?reconnect={platform}`, and supports per-platform retry + manual refresh. In-memory fee cache: 5-minute TTL keyed on `(platform, restaurantId, itemsHash, address)`; busts automatically on any cart/address change; successful fees only (errors never cached).

**What's next (prioritized):**
0. **🔴 FIRST THING NEXT SESSION — validate + ship the live-only-fees refactor** (uncommitted in working tree as of 2026-04-19 night). Four ordered steps:
   1. **Local smoke test**: `npm run dev`, open `/compare` with a cart, walk through: (a) confirm-gate UI appears with delivery address + DD account addresses list, (b) confirm → both platform cards render live fees within ~6s, (c) expire a SL token by deleting `grub-api:authenticatedSession` from Chrome localStorage for the SL profile → re-run compare → SL card should show `session_expired` + "Reconnect Seamless" deep-link to `/settings?reconnect=seamless`, DD card still works, (d) on Settings page, login flow auto-opens for SL, (e) after reconnect → redirect back to `/compare`, (f) partial retry on the error card works, (g) manual "Refresh live fees" button busts cache and re-queries both, (h) change a cart item quantity → confirm-gate resets, old comparison disappears (compareKey changes). Watch server logs for `[Compare]` messages including "Discarding response for stale key" if you change cart mid-flight.
   2. **Commit the fees-refactor**: files touched are `client/src/{api/client.ts, components/ComparisonView.tsx, components/ComparisonErrorCard.tsx (NEW), components/SettingsPage.tsx, hooks/useComparison.ts (DELETED)}`, `server/src/{adapters/types.ts, adapters/doordash/adapter.ts, adapters/seamless/adapter.ts, routes/compare.ts, services/comparison.ts, services/fees.ts, index.ts}`, and `AGENTS.md`. Commit message suggestion: "Live-only fees: typed adapter errors, preflight, address confirm UI, reconnect deep-links, 5-min keyed cache".
   3. **Deploy to Railway**: `railway up` from project root. Watch build for the TS compile step; `llm-matching.ts` has a pre-existing `openai` import error that doesn't block the build.
   4. **Railway smoke test**: same as #1 but against `kortana-web-production.up.railway.app`. Confirm the preflight endpoint (`/api/compare/preflight?restaurantId=...`) returns `accountAddresses` for DoorDash. Confirm the address-mismatch amber banner appears if your cart address is >500m from any DD saved address.
...
4. **Verify Option B end-to-end** — `markExpired` wiring is deployed but hasn't been observed firing live (the Railway redeploy refreshed tokens so no real 401 has hit since). Next time a real 401/403 surfaces from either adapter, confirm `/api/auth/status` flips to `expired` and the Settings page prompts re-login. If it doesn't fire within ~24-48h of normal use, force-expire a token (e.g., invalidate the `grub-api:authenticatedSession` localStorage entry) to smoke-test the path. See `server/src/services/auth-manager.ts:markExpired` and the `onAuthExpired` callback wiring in `server/src/index.ts`.
5. **Upgrade `checkSession()` to validate tokens, not just presence** (Option A from the earlier investigation) — current impl returns `true` whenever localStorage/cookies exist, which is why stale tokens silently misreported as `authenticated`. A lightweight API ping (e.g., Grubhub `/users/me`, DD `getConsumer`) in `checkSession()` would catch expired tokens during the periodic 5-min session monitor instead of waiting for a user-triggered compare to expose them. Option B (shipped) covers the reactive path; Option A would make the proactive path accurate too.
6. **DD per-request address — Option A (full) — capture an address-creation / address-set mutation** — Current live-only flow uses `updateConsumerDefaultAddressV2` to snap DD's account default to the closest already-saved address (Option B). If Dan's Kortana compare-time address doesn't exist on the DD account, DD fees are computed for the nearest saved address (the UI warns the user). For multi-user / public launch, we need to programmatically *add* a new address to a DD account per request. Requires a HAR capture of the DD "add address" flow to find the mutation (probably `createConsumerAddress` or similar), then call it before `setDeliveryAddress` in `DoorDashAdapter.getFees()`.
7. **Cross-platform item matching is leaving 40-55% of items orphaned where data is actually fine** — Smoking-gun case: 10 Ave Finest Deli (`11d76620-63aa-41ab-a79d-50ade35aa5ef`) has SL=191, DD=166 (SL fuller than DD), yet only 94/166 DD items + 86/191 SL items are matched. UI shows the unmatched DD items as "DoorDash only" — wrong; SL items exist that should match. Three root causes from sample diff: (a) **DD canonical_name retains a leading menu-position number** ("10 salsalito turkey" vs SL "salsalito turkey", "13 chicken club" vs "chicken club") — the `cleanItemName()` cleaner strips this for SL but not DD. Fix: extend the leading-number-stripping rule to apply to both platforms in `nameCleaner.ts`. (b) **DD vs SL prices diverge by ~$1 (10-15%)** consistently — combined with borderline name JW scores, the T2 combined threshold (0.78) is missed. The "soft price boost" in T2 may need to tolerate larger price gaps when names are very close. (c) **DD has a "Most Ordered" pseudo-category** that duplicates ~12 items already present in their real category — they should be deduped on ingest (or the matcher needs to skip the second occurrence). Also category names diverge (DD "On the Grill" vs SL "On The Grill", DD "Avocado Sandwich" vs SL "Avocado Sandwiches") — `CATEGORY_NORMALIZATION_MAP` needs case-insensitive comparison and basic singular/plural folding. Re-run `matchMenuItems` after fixes. Investigation script: `server/src/scripts/investigate-ten-ave-matching.ts`.
...
| Fee pricing | Live cart only, no fallback | Cached `platform_fees` and hardcoded estimates removed from comparison engine. Every compare hits both platforms' live `getFees()` → typed error if a platform can't deliver. Cached `platform_fees` column retained for analytics. 5-minute in-memory cache on `(restaurantId, items, address)` for instant re-opens; busts on any cart/address change. |
| DD per-request address | Snap to nearest saved DD account address (Option B) | DD `getFees()` calls `updateConsumerDefaultAddressV2` before building the cart to set the closest saved address for the requested lat/lng. If no saved address is near (>500m), UI warns the user. Full Option A (programmatic address creation via a `createConsumerAddress`-type mutation) is on the backlog for multi-user support — needs HAR capture. |
...
| File | Purpose |
|------|---------|
| `AGENTS.md` | This file. Living project context for any AI coding agent. Update as project evolves. |
| `CLAUDE.md` | Claude Code-specific overrides: includes `@AGENTS.md` plus session/skill/hook guidance unique to that harness. |
