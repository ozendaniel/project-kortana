...
- **Headful Chrome tab cleanup (2026-04-05):** Seamless JS (Rokt ads, Stripe) spawns popup windows during scraping. Three-layer cleanup: (1) pre-scrape closes orphaned tabs from previous runs, (2) page-level `popup` event auto-closes popups during scraping (uses `scrapePage.on('popup')` not `context.on('page')` to avoid closing other workers' tabs), (3) post-scrape cleanup with 500ms delay for propagation. Critical for unattended 24h runs in headful mode.
